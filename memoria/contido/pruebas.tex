...
\hline
Unitarios (Mockito)          & 193 & \texttt{domain}, \texttt{application} & 91,4\% \\
Integración (Testcontainers) & 52  & \texttt{infrastructure} + end-to-end   & 81,1\% \\
Arquitectura (ArchUnit)      & 5   & Reglas de capas                        & N/A    \\
\hline
\textbf{Total}               & \textbf{250} & \textbf{---} & \textbf{85,3\%} \\
\hline
\end{tabular}}
...
La cobertura del 91,4\% en \texttt{domain} y \texttt{application} refleja que
las capas más críticas del sistema están ampliamente verificadas mediante
tests unitarios rápidos. La capa de infraestructura alcanza el 81,1\% gracias
a ocho clases \texttt{*ControllerIT}, una por cada controlador REST
(\texttt{UserControllerIT}, \texttt{CenterControllerIT},
\texttt{FieldControllerIT}, \texttt{BookingControllerIT},
\texttt{TournamentControllerIT}, \texttt{MatchControllerIT},
\texttt{AuthControllerIT}) más \texttt{SecurityConfigPublicEndpointsIT} para
la configuración de seguridad. El hueco restante se concentra en
\texttt{GlobalExceptionHandler} (50,9\%): cada \texttt{@ExceptionHandler}
solo se cubre si algún test llega a provocar esa excepción concreta, y varias
—por ejemplo \texttt{BookingOverlapException} o
\texttt{PairAlreadyRegisteredException}— todavía no tienen un test de
integración dedicado. Cerrar esa brecha por completo queda como trabajo
futuro en la sección~\ref{chap:conclusions}.
