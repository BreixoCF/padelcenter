...
\paragraph{Emparejamientos con número de parejas no potencia de 2} El formato
\texttt{ELIMINATION} necesita, en cada ronda, que el número de parejas se
pueda dividir entre dos hasta llegar a la final. Con un número de parejas que
no es potencia de 2 hacen falta \textit{byes} (pases automáticos a la
siguiente ronda para las parejas sobrantes), lo que complica tanto el cálculo
de quién recibe el \textit{bye} —debe repartirse de forma justa, no siempre al
mismo cabeza de serie— como la generación de los partidos \textit{placeholder}
de rondas posteriores. Tras valorar esa complejidad frente al beneficio real
para un sistema de gestión de un club de pádel, se optó deliberadamente por
\textbf{no} implementarla: \texttt{Close\-Registration\-And\-Generate\-Bracket\-Use\-Case}
exige que el número de parejas confirmadas sea potencia de 2 para
\texttt{ELIMINATION} y rechaza el cierre de inscripción en caso contrario
(sección~\ref{sec:requisitos_funcionales}). Es una decisión consciente de
simplicidad sobre generalidad, no una limitación accidental: soportar byes
queda como posible ampliación futura si la casuística real de los clubes lo
demanda.
...
\begin{description}
  ...
  \item[Cerrar la cobertura de \texttt{GlobalExceptionHandler}:] Como se
    detalla en la sección~\ref{sec:resumen_cobertura}, cada
    \texttt{@ExceptionHandler} solo se cubre si algún test provoca esa
    excepción concreta; faltan tests de integración que fuercen casos como
    \texttt{BookingOverlapException} o \texttt{PairAlreadyRegisteredException}.
    ...
    pertenecía— que tests de integración para los controladores de torneos,
    partidos y autenticación (añadidos después,
    \texttt{TournamentControllerIT}, \texttt{MatchControllerIT} y
    \texttt{AuthControllerIT}) habrían detectado de forma automática mucho
    antes.
\end{description}
