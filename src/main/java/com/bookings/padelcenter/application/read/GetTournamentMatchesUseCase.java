...
public class GetTournamentMatchesUseCase implements QueryUseCase<GetTournamentMatchesQuery, PageResult<Match>> {
	...
	@NonNull
	@Override
	public PageResult<Match> execute(GetTournamentMatchesQuery query) {
		tournamentRepository.findById(query.tournamentId())
				.orElseThrow(() -> new TournamentNotFoundException(query.tournamentId()));
		return matchRepository.findByTournamentId(query.tournamentId(), query.page(), query.size());
	}
}
